...
def map_and_score(transcript, passage):
    ...
    Determine the score using best fit and record the mappings for debugging. """
    maps = []  # for debugging
            ...
            if step >= hit_count_max:  # ensure we're only inpecting chunks >= max observed
                ...
                for word in transcript:
                    if word in chunk:
                        ...
                        map_.append('I')  # placeholder for unexpected insertions
                hit_count = len(hits)
...
def score(transcript, passage):
    """ Just like map_and_score but without recording the mapping to save time. """
    fit_max, hit_count_max = 0, 0
    transcript = transcript.lower().replace('  ', ' ').split()
    passage = passage.lower().replace('  ', ' ').split()
    passage_length = len(passage)
